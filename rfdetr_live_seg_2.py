...
# Событие — только через историю трека: первый переход «не на линии» → «на линии» (или первый кадр трека уже на линии).
# Пока объект стоит на линии с мелкими смещениями — не повторять. После LINE_TRIPWIRE_REARM_FRAMES кадров без
# пересечения можно снова засчитать один новый вход.
LINE_TRIPWIRE_REARM_FRAMES = 15

# Упрощённая логика: «есть door/trim, пересекает линию — фиксируем».
# Событие выдаётся при ПЕРВОМ кадре, на котором у трека line_hit=True. Без fresh_entry/prev_hit/rearm.
# Один трек = одно событие. Несколько треков на одном кадре → каждое своё событие.
LINE_EVENT_SIMPLE_FIRST_HIT_PER_TRACK = True

# Подавление «фрагмента» одной двери, разрезанной детектором на верх/низ в разных треках.
# Очень узкий критерий — режем только когда новый трек гарантированно «внутри» того, по
# которому буквально только что выдали событие.
#   1) разница кадров с последним EVENT ≤ FRAGMENT_RECENT_FRAMES, и
#   2) новый трек явно перекрывается с треком этого последнего события: либо IoU ≥ IOU_THR,
#      либо containment (inter / min(area_new, area_latched)) ≥ CONTAINMENT_THR.
# Иначе вторая реальная дверь, идущая следом, проходит как новое событие.
...
# Привязка LINE_CROSS к person: True — искать ближайшего человека и писать его в JSON/рисовать
# на кадре; False — игнорировать класс person полностью (событие зависит ТОЛЬКО от door/trim).
LINE_EVENT_USE_PERSON_ASSOCIATION = False

# Пересечение tripwire по маске (сегментация): только пиксели маски ∩ полоса линии.
# Bbox не используем при наличии маски — иначе чужой bbox пересекает линию, а маска пола — нет.
TRIPWIRE_MASK_LINE_THICKNESS_PX = 4
TRIPWIRE_MASK_MIN_OVERLAP_PX = 12  # минимум совпадающих пикселей (отсечь шум 1–2 px)

# Одна дверь → разные маски/трека: повторный LINE_CROSS не публикуем, если центр bbox всё еще
# рядом с последним событием door/trim и прошло меньше N кадров (новый трек не = новая дверь).
# Для того же track_id — как раньше, только близость во времени/пространстве.
# Для другого track_id — дополнительно требуется IoU ≥ порога с bbox последнего события (иначе
# вторая физическая дверь в той же зоне кадра не дублирует "тот же объект", но и не глушится).
DOOR_LINE_EVENT_DEDUP_FRAMES = 180
DOOR_LINE_EVENT_DEDUP_CENTER_DIST_PX = 400.0
# Если LINE_CROSS с другого track_id: не считать дубликатом без заметного пересечения bbox
# с последним событием (иначе вторая физическая дверь в той же зоне кадра глушится).
DOOR_LINE_EVENT_DEDUP_MIN_IOU_DIFFERENT_TRACK = 0.12

# Глобальная анти-повторная проверка LINE_CROSS (время / центр / IoU между событиями).
# True — если новое событие близко по времени и пространству к предыдущему, считаем дубликатом
# одной и той же двери (новый трек после потери старого = тот же физический объект).
DOOR_LINE_EVENT_DEDUP_ENABLED = True

# Подробный лог door/trim vs tripwire каждый кадр (трек обновлён).
# Вкл: True ниже, или env RFDETR_DEBUG_DOOR_LINE=1 (и перезапуск скрипта).
...
# Событие: door/trim + пересечение tripwire с учётом истории (см. LINE_TRIPWIRE_REARM_FRAMES, fresh entry).
...
MIN_DOOR_HEIGHT_PX = 320
MIN_DOOR_AREA_PX2 = 70000
# Верхняя граница площади bbox door (px²). 0 = не ограничивать (крупная дверь в кадре допустима).
MAX_DOOR_AREA_PX2 = 0
# Защита от «штанги/прута»: bbox с h/w > MAX_DOOR_ASPECT_HW не считаем дверью.
# Реальная дверь даже под наклоном редко выходит за ~3.0. 0 = выключено.
MAX_DOOR_ASPECT_HW = 3.0
MAX_DOOR_ASPECT_WH = 3.0
...
# Порог уверенности детектора для door/trim. Детекции с conf ниже не попадают в трекинг
# и в события (отрежет «помехи» на ночных кадрах при низком contrast).
MIN_DOOR_CONFIDENCE = 0.30
MIN_TRIM_CONFIDENCE = 0.30

# Доп. защита от «мгновенной» помехи: событие только если у трека уже есть хотя бы N кадров
# истории, и средний confidence по последним EVENT_CLASS_WINDOW записям ≥ порога.
EVENT_MIN_TRACK_HISTORY = 3
EVENT_MIN_MEAN_CONFIDENCE = 0.35

# Доп. защита от «маска чуть зацепила линию»: для выдачи события требуем заметное
# пересечение маски с tripwire — оба условия:
#   - абсолютное число пикселей: mask_overlap_px ≥ EVENT_MIN_MASK_OVERLAP_PX,
#   - относительное к ширине bbox: mask_overlap_px ≥ EVENT_MIN_MASK_OVERLAP_PER_BBOX_W * bbox_w.
# Также маска не должна быть «прозрачной» внутри bbox: mask_area / bbox_area ≥ EVENT_MIN_MASK_FILL_RATIO.
EVENT_MIN_MASK_OVERLAP_PX = 40
EVENT_MIN_MASK_OVERLAP_PER_BBOX_W = 0.15
EVENT_MIN_MASK_FILL_RATIO = 0.20

# Какие классы создают LINE_CROSS события: только door (trim — не сохраняем).
EVENT_PRIMARY_CLASSES = (rv.DOOR_CLASS_NAME,)

# Если bbox/маска "door" значительно лежит ВНУТРИ bbox/маски "person" — это не дверь,
# а помеха (детектор обвёл «дверью» силуэт человека). Симметричный случай —
# person⊂door — НЕ помеха: это нормально, когда человек стоит/проходит в проёме большой
# двери. Поэтому проверяем только направление door→person.
#   - IoU(door, person) ≥ DOOR_PERSON_OVERLAP_IOU_THR И они примерно одного размера
#     (area_door ≤ area_person * DOOR_PERSON_AREA_RATIO_MAX) — bbox-ы практически совпадают;
#   - door⊂person ≥ DOOR_PERSON_OVERLAP_CONTAINMENT_THR — дверь внутри человека;
#   - mask_door ∩ mask_person / area_mask_door ≥ DOOR_PERSON_OVERLAP_MASK_RATIO_THR —
#     основная часть пикселей маски двери — это пиксели человека.
LINE_EVENT_REJECT_DOOR_OVERLAPPING_PERSON = True
DOOR_PERSON_OVERLAP_IOU_THR = 0.45
DOOR_PERSON_OVERLAP_CONTAINMENT_THR = 0.60
DOOR_PERSON_OVERLAP_MASK_RATIO_THR = 0.55
# Защита: если дверь явно больше человека (>= в N раз по площади) — это большая
# реальная дверь с человеком в проёме, не помеха. IoU/containment отключаем.
DOOR_PERSON_AREA_RATIO_MAX = 1.5

# Размер скользящего окна для проверки door≈person. Детектор иногда «моргает» —
# на самом кадре события person может отсутствовать, но на соседних — присутствовать.
# Поэтому проверяем перекрытие door с любыми person, замеченными в окне последних N кадров.
LINE_EVENT_PERSON_OVERLAP_HISTORY_FRAMES = 5

# Защита от статичных объектов (стоящие в кадре ворота/арки/доски «как дверь»).
# Если в окне последних N кадров центр bbox двери почти не сместился И размеры bbox
# почти не изменились — событие не выдаём: реальная пересекающая линию дверь
# хотя бы по одному из критериев двинется. Проверка применяется только когда у трека
# накоплена история ≥ EVENT_MOTION_LOOKBACK_FRAMES, иначе мы просто не успели её оценить.
LINE_EVENT_REQUIRE_MOTION = True
EVENT_MOTION_LOOKBACK_FRAMES = 8
# абсолютные пороги: max смещение центра bbox и max изменение w/h за окно (px)
EVENT_STATIC_CENTER_TRAVEL_PX = 8.0
EVENT_STATIC_SIZE_DELTA_PX = 12.0
# относительные пороги (от max(w,h) первого кадра окна) — для крупных объектов
...
class LiveEventProcessor:
    ...
    def _door_overlaps_any_recent_person(
        self, door_box, door_mask
    ) -> tuple[bool, str]:
        """Проверка door против person_dets за окно последних кадров.
        Возвращает (overlaps, reason) — reason содержит индекс окна, в котором сработало."""
    ...
    def _track_motion_metrics(self, tr: dict) -> dict | None:
        """Возвращает метрики движения трека за окно EVENT_MOTION_LOOKBACK_FRAMES.
        None — если истории пока недостаточно для оценки."""
    ...
    def _track_is_static(self, tr: dict) -> tuple[bool, str]:
        """True, если объект практически не двигается и не меняет размер в окне.
        Если истории мало — возвращаем (False, "")."""
    ...
    def _door_overlaps_person(
        self, door_box, door_mask, person_dets: list
    ) -> tuple[bool, str]:
        """Возвращает (overlaps, reason). Считает «помехой» door-детекцию, чей
        bbox/маска значительно совпадает с любым person в кадре."""
        ...
        if darea <= 0:
            return False, ""
        door_mask_arr = (
            np.asarray(door_mask, dtype=bool) if door_mask is not None else None
        )
        door_mask_area = (
            float(np.count_nonzero(door_mask_arr)) if door_mask_arr is not None else 0.0
        )
        for pdet in person_dets:
            ...
            if not door_much_bigger:
                if iou >= DOOR_PERSON_OVERLAP_IOU_THR:
                    return (
                        True,
                        f"iou={iou:.2f}>={DOOR_PERSON_OVERLAP_IOU_THR:.2f} "
                        f"(area_ratio={area_ratio:.2f})",
                    )
            if cont_door >= DOOR_PERSON_OVERLAP_CONTAINMENT_THR:
                return (
                    True,
                    f"door⊂person={cont_door:.2f}>={DOOR_PERSON_OVERLAP_CONTAINMENT_THR:.2f} "
                    f"(person⊂door={cont_person:.2f}, area_ratio={area_ratio:.2f})",
                )
            if door_mask_arr is not None and "mask" in pdet:
                ...
                if pmask.shape == door_mask_arr.shape and door_mask_area > 0:
                    ...
                    if ratio >= DOOR_PERSON_OVERLAP_MASK_RATIO_THR:
                        return (
                            True,
                            f"mask_ratio={ratio:.2f}>={DOOR_PERSON_OVERLAP_MASK_RATIO_THR:.2f}",
                        )
        return False, ""
    ...
    def _should_skip_duplicate_primary_line_event(
        ...
        """True, если недавно уже было LINE_CROSS по door/trim рядом с тем же местом кадра."""
    ...
    def _find_overlapping_latched_track(self, tr: dict, frame_idx: int) -> dict | None:
        """
        Возвращает трек последнего EVENT, если текущий — это его «фрагмент»:
          - последнее событие было недавно (≤ LINE_EVENT_FRAGMENT_RECENT_FRAMES);
          - тот трек ещё активен в этом кадре и counted_up;
          - bbox текущего трека сильно перекрывается с bbox того (IoU или containment).
        Иначе — None: либо вторая реальная дверь, либо новый заход.
        """
        ...
        if int(last_tid) == int(tr["id"]):
            return None
        latched = next(
            (t for t in self.tracks if int(t["id"]) == int(last_tid)),
            None,
        )
        if latched is None:
            return None
        ...
        if (
            iou >= LINE_EVENT_FRAGMENT_IOU_THR
            or containment >= LINE_EVENT_FRAGMENT_CONTAINMENT_THR
        ):
            return latched
        return None

    def _record_primary_line_event(
        self, frame_idx: int, obj_class: str, obj_box, track_id: int
    ) -> None:
    ...
    def _bbox_geometric_straddles_line(self, box) -> bool:
        """Есть ли у вершин bbox разные знаки расстояния до линии (линия режет bbox), без поля допусков margin."""
        x1, y1, x2, y2 = map(float, box)
        corners = [
            np.array([x1, y1], dtype=np.float32),
            np.array([x2, y1], dtype=np.float32),
    ...
    def _mask_line_overlap_px(self, mask: np.ndarray) -> int:
        """Число пикселей маски, попадающих в полосу tripwire (для логов и порога)."""
        ...
        cv2.line(
            ...
            p1,
            255,
            TRIPWIRE_MASK_LINE_THICKNESS_PX,
            cv2.LINE_8,
        )
        stripe_b = stripe > 0
        mb = m.astype(bool)
        return int(np.count_nonzero(mb & stripe_b))

    def _mask_intersects_tripwire(self, mask: np.ndarray) -> bool:
        """Маска пересекает tripwire: полоса вдоль отрезка (LINE_8, без сглаживания — меньше ложных краёв)."""
    ...
    def _object_intersects_tripwire(self, hist_item: dict) -> bool:
        """Если есть маска сегментации — только она (см. коммент. к TRIPWIRE_*). Иначе — bbox."""
    ...
    def _update_tripwire_leave_streak_and_rearm(self, tr: dict, hist_item: dict) -> None:
        """
        Кадры подряд без пересечения tripwire; после LINE_TRIPWIRE_REARM_FRAMES снимаем counted_up,
        чтобы следующий реальный заход снова засчитался один раз.
        В режиме LINE_EVENT_SIMPLE_FIRST_HIT_PER_TRACK rearm выключен — один трек = одно событие.
        """
    ...
    def _evaluate_track(self, track: dict) -> dict:
        """
        Door/trim пересекает линию по текущему кадру истории; событие только при «свежем входе»
        (до этого кадр был без пересечения, или это первый кадр трека). counted_up лочит повтор
        на той же стоянке; разлочка через _update_tripwire_leave_streak_and_rearm.
        """
        ...
        return {
            ...
            "line_intersects": line_hit,
            "prev_line_intersects": prev_hit,
    ...
    def _log_track_decision(self, track: dict, frame_idx: int, eval_data: dict) -> None:
        ...
        parts = [
            ...
            f"line_hit={int(eval_data.get('line_intersects', False))}",
            f"fresh_touch={int(eval_data.get('fresh_line_touch_ready', False))}",
            f"reason={eval_data['reason']}",
        ]
        if LINE_EVENT_USE_PERSON_ASSOCIATION:
            parts.insert(
                -1,
    ...
    def _log_door_line_frame_debug(self, frame_idx: int, tr: dict, eval_data: dict) -> None:
        """При RFDETR_DEBUG_DOOR_LINE=1: строка на кадр по обновлённому треку door/trim vs tripwire."""
        if not LIVE_DEBUG_DOOR_LINE_PER_FRAME:
            return
        if tr.get("tracking_group") != "door_trim_group":
            return
        ...
        if not hist:
            return
        last_item = hist[-1]
        ow, oh, oarea = rv.box_wh_area(tr["box"])
        recent_cls = self._get_event_class_recent(tr)

        mk = last_item.get("mask")
        if mk is not None and np.asarray(mk).size and np.any(mk):
            mask_px = self._mask_line_overlap_px(np.asarray(mk))
            hit_mode = "mask"
        else:
            mask_px = -1
            hit_mode = "bbox"

        bm = self._box_line_metrics(last_item["box"])
        straddle = self._bbox_geometric_straddles_line(last_item["box"])

        size_ok, size_reason = _is_valid_object_size(recent_cls, tr["box"])

        dedup_would_skip = None
        dedup_iou_vs_last = None
        if eval_data.get("fresh_line_touch_ready"):
            dedup_would_skip = self._should_skip_duplicate_primary_line_event(
                frame_idx, recent_cls, tr["box"], tr["id"]
            )
            if self._last_primary_line_event_box is not None:
                dedup_iou_vs_last = float(
                    rv.iou_xyxy(self._last_primary_line_event_box, tr["box"])
                )

        parts = [
            f"[DOOR LINE DBG] {self.source_name}",
            f"frame={frame_idx}",
            f"track_id={tr['id']}",
            f"updated={int(tr.get('updated_this_frame', False))}",
            f"class_evt={recent_cls}",
            f"class_now={tr.get('class_name_current')}",
            f"lost={tr.get('lost', 0)}",
            f"hist_len={len(hist)}",
            f"obj_w={int(ow)}",
            f"obj_h={int(oh)}",
            f"area={int(oarea)}",
            f"hit_mode={hit_mode}",
            f"mask_overlap_px={mask_px}",
            f"need_px>={TRIPWIRE_MASK_MIN_OVERLAP_PX}",
            f"bbox_state={bm['state']}",
            f"bbox_straddle={int(straddle)}",
            f"line_hit={int(eval_data.get('line_intersects', False))}",
            f"prev_hit={int(eval_data.get('prev_line_intersects', False))}",
            f"fresh_entry={int(eval_data.get('fresh_entry', False))}",
            f"latched={int(eval_data.get('latched', False))}",
            f"leave_streak={eval_data.get('tripwire_leave_streak', 0)}",
            f"reason={eval_data.get('reason')}",
            f"fresh_ready={int(eval_data.get('fresh_line_touch_ready', False))}",
            f"size_ok={int(size_ok)}",
            f"size_reason={size_reason if not size_ok else 'ok'}",
        ]
        if dedup_would_skip is not None:
            parts.append(f"dedup_would_skip={int(dedup_would_skip)}")
        if dedup_iou_vs_last is not None:
            parts.append(f"dedup_iou_vs_last={dedup_iou_vs_last:.3f}")
        self.log(" | ".join(parts))
    ...
    def process_frame(self, frame: np.ndarray, all_dets: list, frame_idx: int) -> np.ndarray:
        ...
        for det in primary_dets:
            ...
            min_conf = (
                ...
                else MIN_TRIM_CONFIDENCE
                if cls_name == rv.TRIM_CLASS_NAME
                else 0.0
            )
            if min_conf > 0.0 and conf < min_conf:
                ow, oh, oarea = rv.box_wh_area(det["box"])
                self.log(
                    f"[SKIP LOW CONF] {self.source_name} | frame={frame_idx} | "
                    f"class={cls_name} | conf={conf:.2f} < {min_conf:.2f} | "
                    f"w={int(ow)} h={int(oh)} area={int(oarea)}"
                )
                continue
        ...
        for det in enriched_primary:
            ...
            if best_track is not None:
                ...
                self._update_tripwire_leave_streak_and_rearm(self.tracks[-1], hist_item)
        ...
        if LIVE_DEBUG_DOOR_LINE_PER_FRAME:
            nd = [d for d in enriched_primary if d.get("tracking_group") == "door_trim_group"]
            ut = [
                t
                for t in self.tracks
                if t.get("tracking_group") == "door_trim_group" and t.get("updated_this_frame")
            ]
            extra = ""
            if len(nd) > len(ut):
                extra = f" | WARN_dets_gt_updated_tracks dets={len(nd)} updated_ids={[t['id'] for t in ut]}"
            self.log(
                f"[DOOR LINE DBG] {self.source_name} frame={frame_idx} SUMMARY | "
                f"door_trim_detections={len(nd)} | active_door_tracks="
                f"{sum(1 for t in self.tracks if t.get('tracking_group') == 'door_trim_group')}"
                f"{extra}"
            )
        ...
        for tr in self.tracks:
            ...
            self._log_door_line_frame_debug(frame_idx, tr, eval_data)
            ...
            if obj_class not in EVENT_PRIMARY_CLASSES:
                if not tr.get("rejected_class_skip", False):
                    ...
                    self.log(
                        ...
                        f"reason=class_not_in_primary_event_classes"
                    )
                continue
            ...
            if EVENT_MIN_MEAN_CONFIDENCE > 0.0:
                ...
                if recent_confs:
                    mean_conf = sum(recent_confs) / len(recent_confs)
                else:
                    mean_conf = 0.0
            ...
            if mk is not None:
                ...
                if mk_arr.size and np.any(mk_arr):
                    ...
                    weak_abs = (
                        EVENT_MIN_MASK_OVERLAP_PX > 0
                        and overlap_px < EVENT_MIN_MASK_OVERLAP_PX
                    )
                    weak_rel = (
                        EVENT_MIN_MASK_OVERLAP_PER_BBOX_W > 0
                        and overlap_per_w < EVENT_MIN_MASK_OVERLAP_PER_BBOX_W
                    )
                    weak_fill = (
                        EVENT_MIN_MASK_FILL_RATIO > 0
                        and fill_ratio < EVENT_MIN_MASK_FILL_RATIO
                    )
                    if weak_abs or weak_rel or weak_fill:
                        if not tr.get("rejected_weak_overlap", False):
                            ...
                            self.log(
                                ...
                                f"class={obj_class} | track_id={tr['id']} | "
                                f"overlap_px={overlap_px} (need ≥ {EVENT_MIN_MASK_OVERLAP_PX}) | "
                                f"overlap/bbox_w={overlap_per_w:.2f} "
                                f"(need ≥ {EVENT_MIN_MASK_OVERLAP_PER_BBOX_W:.2f}) | "
                                f"fill={fill_ratio:.2f} (need ≥ {EVENT_MIN_MASK_FILL_RATIO:.2f}) | "
                                f"obj_w={int(ow)} obj_h={int(oh)}"
                            )
                        continue

            if self._should_skip_duplicate_primary_line_event(
                frame_idx, obj_class, obj_box, tr["id"]
            ):
                if not tr.get("rejected_dedup_line", False):
                    tr["rejected_dedup_line"] = True
                    self.log(
                        f"[REJECT DUP LINE ✖] {self.source_name} | frame={frame_idx} | "
                        f"class={obj_class} | reason=PRIMARY_LINE_EVENT_DEDUP_NEAR_TIME"
                    )
                tr["counted_up"] = True
                continue
            ...
            if other is not None:
                if not tr.get("rejected_fragment_of_latched", False):
                    ...
                    self.log(
                        ...
                        f"reason=FRAGMENT_OF_ALREADY_COUNTED_DOOR"
                    )
                tr["counted_up"] = True
                continue
            ...
            if person_info is not None:
                ...
                log_data["person_area"] = person_info["area"]
            ...
            if rv.SAVE:
                ...
                filename = f"{self.source_stem}_LINE_{assoc_str}_{frame_idx:06d}.jpg"
    ...
    class DetWorker(QThread):
        ...
        def run(self) -> None:
            ...
            for source_idx, current_src in enumerate(sources, start=1):
                ...
                self.log.emit(
                    f"Итог по {_source_label(current_src)}: пересечений tripwire {event_processor.events_count}; "
                    f"отклонено (размер): {event_processor.rejected_count}"
                )
            ...
            if self._jsonl_fp is not None:
                ...
                self._jsonl_fp = None
            self.log.emit(
                f"Итого пересечений tripwire: найдено {total_events}, отклонено по размеру {total_rejected}"
            )
            self.finished_ok.emit()
    ...
    class MainWindow(QMainWindow):
        ...
        def _on_video_opened(self, total_frames: int) -> None:
            ...
            if self._video_total <= 1:
                ...
                self._btn_mid.setEnabled(False)
                return
            self._lbl_seek.setText(f"Кадр 1…{self._video_total} (клик в число — ввод; ползунок — тяни; Enter — перейти)")
